...
class CyberTeens:
    ...
    def get_user(self,s):
        ...
        url = "https://api.fjg360.cn/index.php?m=vote&c=index&a=get_members&openid="+ self.openid
        resp = s.get(url, headers=headers).json()
        if resp.get("code") == 1:
            return resp.get("h5_ask_member")
        else:
            return "您的OPENID配置有误，请检查后重试"

        
    def get_course(self,s,code):
        headers = {
            "Host": "h5.cyol.com",
            "Connection": "keep-alive",
    ...
    def save_door(self,info, course, s):
        """
